...
import {PersonaTableComponent} from '../../empleado-table/persona_table.component';
...
if(this.contactId != null){
 ...
 if(this.contactId == null){
    ...
    const C_Correo = this.form.get("Correo").value;
    const C_Img_Base = this.form.get("c_Img_Base").value;
    const C_Descripcion = this.form.get("Razon").value;
    const C_Fecha_Nacimiento = this.form.get("Fecha_Nacimiento").value;
    const c_Usuario_Modificacion = this.form.get("c_Usuario_CoM").value;

 
   this.valido = true;
    this.CrearUsuariosService.putActualizarPersona(
      C_Id_Persona,
      C_ID_Rol_Persona,
      C_Id_Sucursal,
      C_Id_Genero,
