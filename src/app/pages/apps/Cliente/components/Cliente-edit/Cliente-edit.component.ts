...
VF: boolean = false;
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public contactId: ClienteData['c_Id_Persona'],
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group({
      ...
      c_Tipo_Cuenta: [0,Validators.required],
    ...
    if(this.contactId == null){
      ...
      this.form.get('c_Cuenta')?.reset();
      this.form.get('c_Cuenta')?.disable();
...
if(this.contactId != null){
  ...
  this.form.get('c_Id_Sucursal')?.reset();
  this.form.get('c_Id_Sucursal')?.disable();

    this.CrearUsuariosService.getProveedorById(this.contactId).subscribe(
      (data: any) => {
        this.empleado = data;
      if(data.response.c_Id_Rol_Persona == 2)
      {
        ...
        this.form.patchValue({
          ...
          if(data.response.c_Id_Cuenta !== "F" && this.rol.includes("GERENTE")){
          
            this.Existe = true;
            this.form.get('c_Tipo_Cuenta')?.reset();
            this.form.get('c_Tipo_Cuenta')?.enable();

            this.form.patchValue({ c_Tipo_Cuenta: data.response.c_Id_Tipo_Cuenta
            });
            this.cuentaExists = true;
            this.toggleChecked = true;
            this.buttonText = this.toggleChecked ? 'Con Cuenta' : 'Sin Cuenta';
            this.validar  ="T";
          }else if(data.response.c_Id_Cuenta !== "F" && this.rol.includes("CAJERO")){
            this.form.get('c_Tipo_Cuenta')?.reset();
            this.form.get('c_Tipo_Cuenta')?.disable();
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result != false) {
          ...
          if(this.contactId == null){
             ...
             this.CrearUsuariosService.setCliente(
               C_ID_Rol_Persona,
               C_Validar,
               C_Tipo_Cuenta,
               C_Id_Sucursal,
              ...
              const C_Validar = this.validar;
             const C_Id_Persona = this.form.get("Id_Empleado").value;
             const C_ID_Rol_Persona = 1;
             const c_Id_Tipo_Cuenta = this.form.get("c_Tipo_Cuenta").value !== null ? this.form.get("c_Tipo_Cuenta").value : 0;
             const C_Id_Sucursal = this.form.get("c_Id_Sucursal").value;
             const C_Id_Genero = this.form.get("c_Id_Genero").value;
             const C_Id_Municipio = this.form.get("c_Id_Municipio").value;
             const C_Primer_Nombre = this.form.get("Primer_Nombre").value;
