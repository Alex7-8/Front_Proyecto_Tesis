...
@Component({
  ...
  templateUrl: './marca_menu.component.html',
  animations: [fadeInRight400ms, stagger40ms]
})
export class MarcaTableMenuComponent implements OnInit {
  Marca: MarcaData[] = [];
  ...
  @Output() filterChange = new EventEmitter<MarcaData[]>();
  ...
  constructor(private CrearUsuariosService:CrearUsuariosService,
              private em:MarcaTableComponent,
  ...
  onToggleChange(event: any): void {
    ...
    if (event.checked  &&  this.em.activeCategory == 'Activo') {
      ...
      this.router.navigate(['/apps/marca/grid/inactivos']);
  ...
  setFilter(category: ContactsTableMenu['id']) {
     ...
     if (category === 'Activo') {
      this.em.activeCategory = 'Activo';
      this.em.tableData = this.em.Marca.filter(c => c.c_Estado === 1);

    }
    
    if (category === 'Inactivo') {
      this.em.activeCategory = 'Inactivo';
      this.em.tableData = this.em.Marca.filter(c => c.c_Estado === 2);
    }
  }
